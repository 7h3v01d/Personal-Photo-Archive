...
def _setup(tmp_path: Path, *, files=(('a','p1'),('b','p2')), sha='same'):
    ...
    for fid,pid in files:
        ...
        c.execute("INSERT INTO files(id,photo_id,path,filename,size_bytes,first_seen_at,last_seen_at,library_id,sha256,presence_status,health_status) VALUES (?,?,?,?,1,'2020','2021',?,?,'present','ok')",(fid,pid,str(p),p.name,lid,sha))
        rid='r'+fid
        c.execute("INSERT INTO file_revisions(id,file_id,sha256,size_bytes,first_observed_at) VALUES (?,?,?,1,'2020')",(rid,fid,sha))
...
def test_schema_v26_and_merge_plan_requires_explicit_survivor(tmp_path):
    ...
    p=plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='p1')
    assert p.schema==IDENTITY_MERGE_PLAN_SCHEMA and p.retired_photo_id=='p2' and p.moved_file_ids==('b',)
    with pytest.raises(ValueError,match='survivor'):
        plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='not-a-photo')
...
def test_controlled_merge_reassigns_files_retires_only_loser_and_audits(tmp_path):
    ...
    plan=plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='p1')
...
def test_merge_plan_stales_if_file_state_changes_before_write_lock(tmp_path):
    c,lid,_=_setup(tmp_path)
    plan=plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='p1')
    c.execute("INSERT INTO file_revisions(id,file_id,sha256,size_bytes,first_observed_at) VALUES ('changed','b','same',1,'2022')"); c.execute("UPDATE files SET current_revision_id='changed' WHERE id='b'"); c.commit()
...
def test_merge_refuses_independent_photo_notes_and_organization_history(tmp_path):
    ...
    inv=investigate_competing_identity(c,library_id=lid,sha256='same')
    assert not inv.merge_consideration.eligible and any('notes' in b for b in inv.merge_consideration.blockers)
    with pytest.raises(ValueError,match='not eligible'):
        plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='p1')
...
def test_merge_refuses_prior_identity_history_and_does_not_chain_silently(tmp_path):
    ...
    root=tmp_path/'lib'; p=root/'d.jpg'; p.write_bytes(b'd')
    c.execute("INSERT INTO photos(id,created_at) VALUES ('p3','2022')")
    c.execute("INSERT INTO files(id,photo_id,path,filename,size_bytes,first_seen_at,last_seen_at,library_id,sha256,presence_status,health_status) VALUES ('d','p3',?,'d.jpg',1,'2022','2022',?,'same','present','ok')",(str(p),lid))
    c.execute("INSERT INTO file_revisions(id,file_id,sha256,size_bytes,first_observed_at) VALUES ('rd','d','same',1,'2022')")
...
def test_merge_never_changes_evidence_or_source_bytes(tmp_path):
    ...
    execute_identity_merge(c,plan_identity_merge(c,library_id=lid,sha256='same',survivor_photo_id='p1'))
